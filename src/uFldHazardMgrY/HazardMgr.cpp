...
bool HazardMgr::handleMailSensorConfigAck(string str)
{
  ...
  if(valid_msg) {
    ...
    m_pH_H=m_pc_granted/(m_pc_granted+m_pfa_granted);
    m_pH_B=(1-m_pc_granted)/(2-m_pc_granted-m_pfa_granted);
  }
...
void HazardMgr::handleClassifyRequest(string response)
{
  ...
  string label = parsed_response[2];
  biteStringX(label, '=');
  string type = parsed_response[3];
  biteStringX(type, '=');
  bool same;
  ...
  same=new_hazard.getLabel()==label;
  ...
  if(ix == -1){
    ...
    new_prob.id=label;
    if(type  == "hazard"){
      new_prob.times_hazard=1;
      new_prob.times_benign=0;
    }
    else{
      new_prob.times_hazard=0;
      new_prob.times_benign=1;
    }
    m_prob_by_id.push_back(new_prob);
    Notify("HAZARD_STAT", "Added a Hazard: " + new_hazard.getSpec(""));
    reportEvent("Added a Hazard");
    //Notify("VISIT_POINT", parsed_report);
  }
  else{
    Notify("HAZARD_STAT", "size of probability list is: "+to_string(m_prob_by_id.size()));
    for(int i=0; i<m_prob_by_id.size(); i++){
      ProbByID temp=m_prob_by_id[i];
      if(label==temp.id){
	if(type=="hazard")
	  temp.times_hazard+=1;
	else
	  temp.times_benign+=1;
	if(temp.times_hazard<temp.times_benign){
	  new_hazard.setType("benign");
	  Notify("HAZARD_STAT", "reset type to BENIGN");
	  reportEvent("reset type to BENIGN");
	}
	else{
	  new_hazard.setType("hazard");
	  Notify("HAZARD_STAT", "reset type to HAZARD");
	  reportEvent("reset type to Hazard");
	}
      }			   
    }
    m_hazard_set.setHazard(ix, new_hazard); // in the case that we have already seen it
    Notify("HAZARD_STAT", "Updated a Hazard: " + new_hazard.getSpec(""));
  }
  string event = "New classification, label=" + new_hazard.getLabel();
  event += ", x=" + doubleToString(new_hazard.getX(),1);
  event += ", y=" + doubleToString(new_hazard.getY(),1);
  event += ", match=" + boolToString(same);
  reportEvent(event);
  Notify("HAZARD_STAT", event);
}
...
{
  Notify("HAZARD_STAT", "got NODE_MESSAGE: PARTNER_HAZARDS");
  reportEvent("got NODE_MESSAGE: PARTNER_HAZARDS");
  string temp = hazards;
    Notify("HAZARD_STAT", "Recieved Hazards: " + hazards);
    vector<string> hazard_list= parseString(temp, '#');
    for(int i=0;i<hazard_list.size();i++){
      string hazard= hazard_list[i];
      string parsed_report ="";
	vector<string> broken_string = parseString(hazard, ',');
	for(int j=0;j<broken_string.size();j++){
      ...
      XYHazard new_hazard= string2Hazard(hazard_list[i]);
      string label= new_hazard.getLabel();
      string type = new_hazard.getType();
      int ix = m_hazard_set.findHazard(label);
      if(ix == -1){
	m_hazard_set.addHazard(new_hazard);//in the case that we have not already seen it
	ProbByID new_prob;
	...
	Notify("HAZARD_STAT", "Updated a Hazard: " + new_hazard.getSpec(""));
      ...
      reportEvent("Got Partner's Hazard");
    }
}
//---------------------------------------------------------
...
void HazardMgr::handleMailReportRequest()
{
  if(!m_reporter){
    ...
    Notify("HAZARDSET_STAT", "REPORT: "+ summary_report);
...
void HazardMgr::handleMailMissionRequest()
{
  if(m_reporter){
    ...
    int init =start;
    int temp = start;
    //if(start<m_haz_to_send.size()){
      for(int i=start; i<min((int)m_haz_to_send.size(), 2+temp);i++){
	string hazard= m_haz_to_send[i];
	vector<string> broken_string = parseString(hazard, ',');
	if(i!=init){
	  parsed_report+="#";
	}
	for(int j=0;j<broken_string.size();j++){
	  string value= broken_string[j];
	  Notify("HAZARD_STAT", "adding element "+ value+" to parsed_report");
	  string key = biteStringX(value,'=');
	  if(key=="x"){
	    parsed_report+="x="+ value+",";
	  }
	    ...
	    parsed_report+="label="+value+",";
	  }
	  else if (key == "type"){
	    parsed_report+="type="+value;
	  }
	}
	if(start <= m_haz_to_send.size()-1)
	  start+=1;
      }
      if(start>=m_haz_to_send.size()-1){
	start=0;
      }
    Notify("HAZARD_STAT", "start = "+ to_string(start));
