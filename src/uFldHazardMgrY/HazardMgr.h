...
class HazardMgr : public AppCastingMOOSApp
{
 ...
 protected: // Registration, Configuration, Mail handling utils
   ...
   void handleMailPartnerHazards(std::string);
   void handlePartnerConfirm();
   void handleMailMissionRequest();
   void handleClassifyRequest(std::string);
 ...
 private: // State variables
   ...
   unsigned int m_intervehicle_reports;
   ...
   double m_pH_H;
   double m_pH_B;
   vector<ProbByID> m_prob_by_id;

   XYHazardSet m_hazard_set;
   XYPolygon   m_search_region;
   list<XYHazard> m_haz_to_process;
    
   double      m_transit_path_width;
   vector<std::string> m_haz_to_send;

   string m_dest_name;
   bool m_pending;
   string m_last_partner_report;
   string m_most_recent_report;
   bool m_confirmation;
   int count;
   int m_sent;
   int m_recieved;
