...
bool GenPath::Iterate()
{
  ...
  Notify("VISIT_SIGNAL", "start stream");
  if((m_point_list.size() <= m_points_processed) &&  m_end && !m_done){
  ...
  //add points to vertex list in order to minimize total distance
  else if(m_point_list.size() != 0  && m_end && !m_done && m_point_list.size() != m_used.size()){
    Point current_point= m_start_point;
	...
	if(current_point.distance_to(new_point)< current_point.distance_to(min_point) && !new_point.in(m_used)){
      ...
      if(current_point.point_string != m_start_point.point_string)
...
bool GenPath::OnStartUp()
{
  ...
  if(m_MissionReader.GetConfiguration(GetAppName(), sParams)) {
	...
	vector<string> xy = parseString(value, ',');
	string rep = "x="+xy[0]+",y="+xy[1]+",id=0";
	Notify("PATH_STAT","START_POINT = "+ rep);
	Point new_point(rep);
