...
HazardMgr::HazardMgr()
{
  ...
  m_pending=false;
  m_last_partner_report="";
  m_most_recent_report="";
  count=0;
  m_recieved=0;
  m_sent=0;
}
...
bool HazardMgr::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
	...
	handlePartnerConfirm();  // TODO4  verify that partner has recieved sent packets and delete them from m_haz_to_send
      Notify("HAZARD_STAT", "got NODE_MESSAGE: "+subkey);
      reportEvent("got NODE_MESSAGE:" + subkey);
    }
...
{
  int i=0;
  while(m_haz_to_send.size()>0 && i<4 && m_sent>m_recieved){
    m_haz_to_send.pop_back();
    i++;
  }
  m_recieved=m_sent;
  reportEvent("m_received = " + to_string(m_recieved));
}
//---------------------------------------------------------
...
bool HazardMgr::Iterate()
{
  ...
  if((m_sent<=m_recieved+1) and count%60==0){
    handleMailMissionRequest();
  }
  else{
     handleMailPartnerHazards(m_most_recent_report);
  }
    count++;
...
{
  string temp = hazards;
  if(hazards != m_last_partner_report){
    ...
    node_message.setStringVal(to_string(m_intervehicle_reports));
...
void HazardMgr::handleMailMissionRequest()
{
  ...
  for(int i=1; i<=min((int)m_haz_to_send.size(), 4);i++){
  ...
  m_pending = true;
  if(parsed_report != m_last_sent){
    m_sent+=1;
  }
  m_last_sent = parsed_report;
  NodeMessage node_message;
  node_message.setSourceNode(m_host_community);
  node_message.setDestNode("all");
  ...
  string msg = node_message.getSpec();
  reportEvent("m_sent = "+to_string(m_sent));
  Notify("NODE_MESSAGE_LOCAL", msg);
}
//---------------------------------------------------------
